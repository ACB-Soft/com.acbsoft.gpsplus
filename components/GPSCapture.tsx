...
const GPSCapture: React.FC<Props> = ({ onComplete, onCancel, isContinuing = false, existingLocations }) => {
  ...
  // İzin denetimi kaldırıldı çünkü Onboarding aşamasında zaten yapılıyor.
  ...
  useEffect(() => {
    if (step === 'READY' || step === 'COUNTDOWN') {
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (err) => { 
            console.error("Konum izleme hatası:", err);
  ...
  const processSamples = useCallback(() => {
    ...
    if (samples.length === 0) {
      alert("Hata: Konum verisi henüz stabilize olmadı.");
      setStep('READY');
      return;
    }
    const avg = {
